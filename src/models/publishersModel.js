...
const createPublisher = async (name) => {
    const result = await pool.query("INSERT INTO publishers (name) VALUES ($1) RETURNING *", [name]);
    return result.rows[0];
};

const updatePublisher = async (id, name) => {
    const result = await pool.query("UPDATE publishers SET name = $1 WHERE id = $2 RETURNING *", [name, id]);
    return result.rows[0];
};
